...
- [ ] 3.0 Implement core math library (traces to: FR-2, FR-7, FR-8, FR-9, FR-10, FR-11, FR-12, FR-13, FR-14, FR-15, FR-16)
  ...
  - [ ] 3.2 Create `src/lib/amortization.ts` — implement `calculateMonthlyPaymentCents(principalCents: number, annualRate: number, amortizationYears: number): number` using Canadian semi-annual compounding formula: `r = (1 + annualRate/2)^(1/6) - 1`, `M = P × [r(1+r)^n] / [(1+r)^n - 1]`, result rounded to nearest cent (FR-2)
  ...
  - [ ] 3.4 Create `src/lib/replay.ts` — implement `replayEvents(events: LoanEvent[], loan: Loan, convention: DayCountConvention): LedgerState` as a pure function. Sort events by date ascending before walking (daysBetween tiebreaker: insertion index). Wrap in `useMemo` at the call site in App.tsx: `useMemo(() => replayEvents(events, loan, convention), [events, loan, convention])` (FR-11)
  - [ ] 3.5 In `replay.ts` — implement funding event handler: call `toCents(event.amount)` to get opening balance in cents, create first ledger row with full principal, no interest or principal split
  - [ ] 3.6 In `replay.ts` — implement payment event handler: call `toCents(event.amount)` to get payment in cents, calculate interest since last event date using `calculateInterestCents`, split: `principalCents = paymentCents - interestCents`, `newBalanceCents = previousBalanceCents - principalCents`. If `principalCents < 0` (interest exceeds payment — deeply delinquent loan), allow it and flag `isNegativePrincipal: true` on the ledger row — do not throw or block (PRD OQ-4) (FR-10, FR-12)
  - [ ] 3.7 In `replay.ts` — implement additional_advance event handler: call `toCents(event.amount)`, increase balance, create ledger row with zero interest/principal split (FR-15)
  - [ ] 3.8 In `replay.ts` — implement payment_reversal event handler: find original payment row by `reversesEventId`, back out its exact `principalCents` and `interestCents`, restore balance, mark original row as `isReversed: true`, create reversal ledger row. Interest window for the next event starts from the reversal event date (FR-13, FR-14)
  - [ ] 3.9 In `replay.ts` — implement payoff event handler: calculate per-diem interest from last event to payoff date, create final ledger row, set balance to zero (FR-16, FR-18)
  - [ ] 3.10 In `replay.ts` — implement `calculatePayoffQuote(events: LoanEvent[], loan: Loan, convention: DayCountConvention, asOfDate: string): number` — returns payoff amount in cents as of any given date. This is a non-mutating read — it does NOT post a payoff event. The "Payoff Today" stat card calls this function with today's date (FR-16, FR-17)
  - [ ] 3.11 In `replay.ts` — implement `getReversibleEvents(events: LoanEvent[]): LoanEvent[]` — returns only payment and additional_advance events that have not been reversed, for use in the reversal dropdown
  - [ ] 3.12 Manually verify NSF scenario math: funding Jan 1 → payment Feb 1 → reversal Feb 8 → payment Mar 1. Confirm balances match the example in FR-13/FR-14 to the exact cent. Fix any discrepancies before proceeding.
