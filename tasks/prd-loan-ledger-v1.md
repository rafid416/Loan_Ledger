...
**Version:** v1  
**Date:** 2026-06-22  
**Source:** loan-ledger-project.md + clarifying session  
...
Three vertical regions on one page:

1. **Loan Setup** (top or left panel) — form inputs for loan terms + create/reset button
2. **Add Event** (middle panel) — event type selector + contextual fields + submit
3. **Loan State + Ledger** (main panel) — summary cards + transaction table

**Ledger table columns:**
`Date | Type | Amount | Interest | Principal | Balance After`

**Reversed payments:** struck-through row with muted styling. Reversal row shown immediately below with `payment_reversal` type.

**NSF UX:** Clicking a payment row in the ledger populates the reversal dropdown in the Add Event form. Dropdown also usable independently.

Full design decisions to be captured in `drd-loan-ledger-v1.md`.
...
**Day-count:** Actual/365 default. Days between dates calculated using `Math.floor((dateB - dateA) / (1000 × 60 × 60 × 24))`.
